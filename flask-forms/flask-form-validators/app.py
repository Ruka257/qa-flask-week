...
app = Flask(__name__)
app.config['SECRET_KEY']='SOME_KEY' #configure a secret key for CSFR protection.

#create validator
class UserCheck:
    def __init__(self, banned, message=None):# Set up the class to have the banned and message attributes. banned must be passed through at declaration.
        self.banned = banned
        if not message:
...
class myForm(FlaskForm):
    username = StringField('Username', validators=[
        DataRequired(),
        UserCheck(message="That username is not allowed", banned = ['root','admin','sys']), #'data' objects in list in field = banned
        Length(min=2,max=15)
        ])
